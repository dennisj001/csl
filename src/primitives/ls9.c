...
void
expect ( char *who, char *what, cell got )
{
    char b[100] ;
...
readc ( void )
{
    ...
    else
    {
        ...
        return l9_getChar ( Ports[Inport] ) ; //getc ( Ports[Inport] ) ;
    }
}
...
rejectc ( int c )
{
    ...
    {
        l9_ungetChar ( c, Ports[Inport] ) ; //ungetc ( c, Ports[Inport] ) ;
    }
}
...
l9_fwrite ( char *s, int k )
{
    ...
    {
        n = fwrite ( s, 1, k, Ports[Outport] ) ;
        //strncpy ( _O_->Pbf8, s, 15 ) ;
    }
    return n ;
}
...
mark ( cell n )
{
    ...
    while ( 1 )
    {
        if ( specialp ( n ) || ( l9_tag ( n ) & MARK_TAG ) )
        {
            if ( NIL == parent )
                break ;
            if ( l9_tag ( parent ) & VECTOR_TAG )
            { /* S1 --> S1|done */
                i = vecndx ( parent ) ;
                v = vector ( parent ) ;
                if ( l9_tag ( parent ) & TRAV_TAG &&
                    i + 1 < veclen ( parent )
                    )
                { /* S1 --> S1 */
                    x = v[i + 1] ;
...
cell
cons3 ( cell pcar, cell pcdr, int ptag )
{
    cell n ;
    int k ;
...
#define mkchar(c) mkatom(T_L9_CHAR, mkatom((c) & 0xff, NIL))
...
int
open_outport ( char *path, int append )
{
    int i ;
...
P_whitec, P_writec, P_csl_return ;
...
meta ( void )
{
    ...
    switch ( cmd )
    {
        case 'c': cmdsym = symref ( "syscmd" ) ;
            break ;
        case 'h': cmdsym = symref ( "help" ) ;
            break ;
        case 'l': cmdsym = P_load ;
            break ;
        default: prints ( ",c = syscmd" ) ;
            nl ( ) ;
            prints ( ",h = help" ) ;
            nl ( ) ;
            prints ( ",l = load" ) ;
...
prex ( int sl, cell x, int d )
{
    ...
    else if ( ctagp ( x ) ) prints ( "#<catch l9_tag>" ) ;
...
void
fixover ( char *who, cell x, cell y )
{
    char b[100] ;
...
sless ( cell x, cell y )
{
    ...
    return scomp ( x, y ) < 0 ? L9_TRUE : NIL ;
}
...
cell
siless ( cell x, cell y )
{
    if ( ! string ( x ) ) expect ( "si<", "string", x ) ;
    if ( ! string ( y ) ) expect ( "si<", "string", y ) ;
    return scomp_ci ( x, y ) < 0 ? L9_TRUE : NIL ;
}

cell
silteq ( cell x, cell y )
{
    if ( ! string ( x ) ) expect ( "si<=", "string", x ) ;
    if ( ! string ( y ) ) expect ( "si<=", "string", y ) ;
    return scomp_ci ( x, y ) <= 0 ? L9_TRUE : NIL ;
...
existsp ( char *s )
{
    ...
    f = fopen ( s, "r" ) ;
    if ( f != NULL ) fclose ( f ) ;
    return NULL == f ? NIL : L9_TRUE ;
}

cell
openfile ( cell x, int mode )
{
    int p ;
...
dumpimg ( char *path )
{
    ...
    strncpy ( m.version, L9_VERSION, sizeof (m.version ) ) ;
...
char *
xfread ( void *buf, int siz, int n, FILE * f )
{
...
loadimg ( char *path )
{
    ...
    if ( memcmp ( m.version, L9_VERSION, sizeof (m.version ) ) )
    {
        fclose ( f ) ;
    ...
    for ( Run = 1 ; Run ; )
    {
        switch ( ins ( ) )
        {
            ...
            case OP_ALPHAC:
                ...
                Acc = isalpha ( charval ( Acc ) ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_ATOM:
                Acc = pairp ( Acc ) ? NIL : L9_TRUE ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_CAR:
            ...
            case OP_CLOSE_PORT:
                ...
                break ;
            case OP_CONSTP:
                Acc = constp ( Acc ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_CTAGP:
                Acc = ctagp ( Acc ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_DELETE:
            ...
            case OP_FORMAT:
                ...
                break ;
            case OP_FUNP:
                Acc = closurep ( Acc ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_INPORTP:
                Acc = inportp ( Acc ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_LISTSTR:
            ...
            case OP_LOWERC:
                ...
                Acc = islower ( charval ( Acc ) ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_MX:
                Acc = expand ( Acc, 1 ) ;
            ...
            case OP_NUMERIC:
                ...
                Acc = isdigit ( charval ( Acc ) ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_OPEN_INFILE:
            ...
            case OP_OPEN_OUTFILE:
                ...
                break ;
            case OP_OUTPORTP:
                Acc = outportp ( Acc ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_PAIR:
                Acc = pairp ( Acc ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_PEEKC:
            ...
            case OP_UPPERC:
                ...
                Acc = isupper ( charval ( Acc ) ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_VCONC:
                Acc = vconc ( Acc ) ;
            ...
            case OP_WHITEC:
                ...
                Acc = whitespc ( charval ( Acc ) ) ? L9_TRUE : NIL ;
                skip ( ISIZE0 ) ;
                break ;
            case OP_BITOP:
