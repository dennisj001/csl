...
int32 CalculateDispForCallOrJump(byte *insnAddr, byte *jmpToAddr, byte insn, byte insnType);
int32 SetOffsetForCallOrJump(byte *insnAddr, byte *jmpToAddr, byte insnType, byte insn);
int32 _CalculateDispForCallOrJumpOrJcc(BlockInfo *bi);
int32 _SetOffsetForCallOrJump(BlockInfo *bi);
...
void GotoInfo_ResetOffsetAndRemove(GotoInfo *gi, byte *address, Boolean removeFlag);
...
BlockInfo *BI_Block_Copy(BlockInfo *bi, byte *dstAddress, byte *srcAddress, int64 bsize, int64 control);
...
int64 _Udis_Disassemble(ud_t *ud, Word *word, byte *iaddress, int64 number, byte *prefix, byte *postfix, int64 cflag);
...
Rllafl *ReadLiner_LookAroundFor_Logic(uint64 la_code);
Rllafl *Rllafl_New(int64 la_code, uint64 allocType);
