...
TD_PostStructUnion_Accounting ( )
{
    ...
    {
        if ( GetState ( jf_tdi, ( TDI_UNION | TDI_STRUCT ) ) )
        {
    ...
    if ( Is_DebugOn )
    {
        iPrintf ( "\nTypedef : %s size = %d", ctdi->Tdi_StructureUnion_Namespace ? ctdi->Tdi_StructureUnion_Namespace->S_Name : ( byte* ) "", ctdi->Tdi_StructureUnion_Size ) ;
        Pause ( ) ;
    }
#endif             
}
...
#if TD_DEBUG            
        if ( Is_DebugOn ) //&& ( tdi->Tdi_Field_Size < 8 ) )
        {
            iPrintf ( "\nField : %s = %d : Tdi_StructureUnion_Size = %d ", tdi->FieldName, tdi->Tdi_Field_Size, tdi->Tdi_StructureUnion_Size ) ;
            //Pause ( ) ;
        }
#endif             
...
TD_Type_Id ( int64 t_type )
{
    ...
    {
        //if ( String_Equal ( tdi->FileName, "namespaces/types.csl" ) ) 
        //    type0 = Namespace_Find_InOneTree ( tdi->Tdi_InNamespace, token ) ;
        //else 
...
int64
TD_IdentifierField ( int64 t_type )
{
    TDI * tdi = TDI_GetTop ( ) ;
    byte *token = tdi->TdiToken ;
    Namespace * ns ;
    if ( token && ( token [0] != ';' ) )
    {
        ...
        {
            TD_CheckForPointer ( ) ;
            if ( ( tdi->TdiToken [0] != ',' ) && ( tdi->TdiToken [0] != '[' ) && ( tdi->TdiToken [0] != ':' ) )
            {
...
TD_Identifier_Fields ( int64 t_type )
{
    ...
    while ( tdi->TdiToken[0] != ';' )
    {
        ...
        if ( String_Equal ( tdi->TdiToken, "const" ) )
        {
            TDI_ReadToken ( ) ; // register somewhere?
            token = tdi->TdiToken ;
            continue ;
        }
        t_type = TD_IdentifierField ( t_type ) ;
        if ( tdi->TdiToken )
        {
            if ( tdi->TdiToken[0] == ',' )
            {
...
TD_Identifier ( int64 t_type )
{
    ...
    else if ( t_type & ( TD_TYPE_FIELD | TD_FIELD_ID | TD_FUNCTION_ID_FIELD | TD_BIT_FIELD ) )
    {
        ...
        t_type = ( t_type == TD_FUNCTION_ID_FIELD ) ? TD_FUNCTION_ID_FIELD : t_type ;
        //tdi->Tdi_Field_Object = id = DataObject_New ( TD_TYPE_FIELD, 0, identifier, 0, 0, 0, tdi->Tdi_Offset, tdi->Tdi_Field_Size, addToNs, 0, 0, 0 ) ;
        tdi->Tdi_Field_Object = id = DataObject_New ( t_type, 0, identifier, 0, 0, 0, tdi->Tdi_Offset, tdi->Tdi_Field_Size, addToNs, 0, 0, 0 ) ;
...
TDI_Init ( TypeDefInfo * tdi )
{
    ...
    tdi->FileName = _ReadLiner_->Filename ;
    ;
}
...
TDI_Finalize ( )
{
    ...
    if ( ! GetState ( tdi, TDI_PRINT ) ) Object_Size_Set ( rns = ns ? ns : tdi->Tdi_InNamespace, tdi->Tdi_StructureUnion_Size ) ;
    SetState ( _Context_, TDI_PARSING, false ) ;
#if TD_DEBUG            
    if ( Is_DebugOn )
    {
        iPrintf ( "\nTypedef : %s size = %d", rns->S_Name, rns->ObjectByteSize ) ;
        Pause ( ) ;
    }
#endif             
