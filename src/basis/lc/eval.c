...
{
    ListObject *lfirst ;
    _LC_->L1 = _LC_->L0 = LO_CopyOne ( l0 ) ;
    _LC_->ApplyFlag = applyFlag ; //= ! GetState (_LC_, LC_DEFINE_MODE);
    _LC_->Locals = locals ;
    ...
    ///_LC_EvalSymbol ( )
    if ( _LC_->L0 && ( ! LO_IsQuoted ( _LC_->L0 ) ) )
    {
...
start:
            ...
            if ( lfirst )
            {
                ...
                {
                    ListObject *lfunction = LC_Eval ( lfirst, _LC_->Locals, _LC_->ApplyFlag ) ;
                    _LC_->Largs0 = _LO_Next ( lfirst ) ;
                    ///_LC_EvalList ( )
